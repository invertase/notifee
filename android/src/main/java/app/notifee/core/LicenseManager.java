...
class LicenseManager {
  ...
  static void initialize() {
    if (getInstance().mServerPublicKey == null) {
      getInstance().mServerPublicKey =
    ...
    if (getInstance().mClientPrivateKey == null) {
      getInstance().mClientPrivateKey =
  ...
  private static String buildVerifyLicenseRequestToken(String jwtLicenseKey) {
    ...
    try {
      jwtBuilder.signWith(
          loadPrivateKey(getInstance().mClientPrivateKey), SignatureAlgorithm.RS384);
    } catch (Exception e) {
      Logger.e(TAG, "", e);
      return "";
    }
